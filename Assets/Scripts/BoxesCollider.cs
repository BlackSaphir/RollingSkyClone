...
public class BoxesCollider : MonoBehaviour
{
    ...
    public bool CheckIfCollisionBoxes(GameObject Sphere, GameObject Self)
    {
    ...
    void Update()
    {
        ...
        OnCollision = collisionCheck.CheckIfCollisionBoxes(Sphere, this.gameObject);
        ...
        if (OnCollision)
        {
            if (this.gameObject.layer == 8)
            {
                ...
                {
                    Sphere.GetComponent<SphereCollision>().Fallingspeed = 0;
                    Sphere.transform.Translate(new Vector3Self(transform.position.x, transform.position.y + JumpSpeed * Time.deltaTime, transform.position.z));
                }
            }
